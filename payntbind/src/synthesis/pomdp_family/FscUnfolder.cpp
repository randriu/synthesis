...
namespace synthesis {
    ...
    void FscUnfolder<ValueType>::buildStateSpace(
        ...
        while(true) {
            ...
            if(state_action == invalidAction()) {
                ...
                for(const auto& [action,_] : action_prob[memory][observation]) {
                ...
                for(uint64_t choice: this->state_action_choices[state][state_action]) {
                    for(auto const &entry: this->quotient.getTransitionMatrix().getRow(choice)) {
                        ...
                        for(const auto& [action_update,_] : transitions[memory][observation]) {
            ...
            if(action == invalidAction()) {
                ...
                for(uint64_t choice: this->state_action_choices[state][action]) {
                    ...
                    for(auto const &entry: this->quotient.getTransitionMatrix().getRow(choice)) {
                        uint64_t state_dst = entry.getColumn();
                        // executing memory update
                        for(auto const& [action_update,prob]: transitions[memory][observation]) {
                            auto [a,memory_dst] = action_update;
                            if(a == action) {
    ...
    void FscUnfolder<ValueType>::applyFsc(
        ...
        std::vector<std::vector<std::map<uint64_t,double>>> action_prob(transitions.size());
        for(uint64_t n = 0; n < transitions.size(); ++n) {
            action_prob[n].resize(transitions[n].size());
            for(uint64_t z = 0; z < transitions[n].size(); ++z) {
                for(auto const& [action_update,prob]: transitions[n][z]) {
                    auto [action,update] = action_update;
