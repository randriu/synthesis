...
def main():
    ...
    for filename in os.scandir(directory):
        if filename.is_file():
            head, tail = os.path.split(filename.path)
            new_filepath = os.path.join('/home/vojta/FIT/DP/synthesis/outputs', tail + ".txt")
            f = open(new_filepath, "w")
            os.system(f"timeout 7200s python3 paynt.py --project {head} --sketch {tail} --pomdp-memory-size 1 --fsc-synthesis | tee {new_filepath}")
            print("head",head)
