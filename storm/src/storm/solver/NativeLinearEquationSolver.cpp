...
namespace storm {
    namespace solver {
        ...
        template<typename ValueType>
        NativeLinearEquationSolver<ValueType>::CSRMatrix::CSRMatrix(storm::storage::SparseMatrix<ValueType> const& A, const unsigned int nnzInWG) {
        ...
        template<typename ValueType>
        bool NativeLinearEquationSolver<ValueType>::solveEquationsCudaJacobi(Environment const& env, std::vector<ValueType>& x, std::vector<ValueType> const& b) const {
            STORM_LOG_INFO("Solving linear equation system (" << x.size() << " rows) with NativeLinearEquationSolver (CUDA Jacobi)");
            ...
            CSRMatrix LUMatrix(jacobiDecomposition->LUMatrix, NNZ_PER_WG);
            ...
            result = this->solveEquationsJacobi(env, x, b);
#endif
        ...
        bool NativeLinearEquationSolver<ValueType>::internalSolveEquations(Environment const& env, std::vector<ValueType>& x, std::vector<ValueType> const& b) const {
            switch(getMethod(env, storm::NumberTraits<ValueType>::IsExact || env.solver().isForceExact())) {
                ...
                case NativeLinearEquationSolverMethod::CudaJacobi:
                    return this->solveEquationsCudaJacobi(env, x, b);
            }
