...
class Statistic:
    ...
    def start(self, parameter_space: Any) -> None:
        logger.info(f"synthesis initiated, design space: {parameter_space.size_or_order}")
    ...
    def new_fsc_found(self, value: Any, assignment: Any, size: int) -> None:
        round(self.synthesis_timer_total.read(), 1)
    ...
    def get_summary(self) -> str:
        ...
        design_space = (
            f"number of holes: {self.colored_mdp.parameter_space.num_parameters}, "
            f"parameter space size: {self.colored_mdp.parameter_space.size_or_order}, "
            f"underlying MDP: {underlying_mdp_states} states / {underlying_mdp_actions} actions"
        )
