...
def reduce_constraint_to_optimality(
    specification: paynt.specification.property.Specification,
) -> tuple[paynt.specification.property.Specification, paynt.specification.property.Property]:
    """
        ...
        for choice in range(mdp_nci[state], mdp_nci[state + 1]):
            ...
            for choice in range(mdp_nci[state], mdp_nci[state + 1]):
                if dt_colored_mdp_factory.action_labels[dt_colored_mdp_factory.choice_to_action[choice]] == "__random__":
                    ...
                    break
            continue
            raise AssertionError(f"no choice for state {state} even though action {chosen_action_label} was chosen")
...
def get_submdp_from_unfixed_states(dt_colored_mdp_factory: Any, unfixed_states: Any = None) -> Any:
    ...
    return dt_colored_mdp_factory.build_from_choice_mask(selected_choices)
...
def scikit_tree_to_tree_helper(clf: Any, variables: list[Any], action_labels: list[str]) -> list[dict[str, Any]]:
    ...
    for i in range(num_nodes):
        ...
        helper.append(
            {"id": i, "leaf": False, "chosen": (variable, threshold), "children": [int(clf.tree_.children_left[i]), int(clf.tree_.children_right[i])]}
        )
...
def run_scikit_learn_tree(
    ...
    return scikit_tree_to_tree_helper(clf, variables, adjusted_action_labels)
