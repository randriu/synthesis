...
class PomdpFamilyColoredMdp(FamilyColoredMdp):
    ...
    def __init__(
        ...
        super().__init__(
            underlying_mdp, parameter_space, coloring, use_exact, num_actions, action_labels, choice_to_action, state_action_choices, state_to_actions
        )
    ...
    def build_dtmc_sketch(self, fsc: paynt.pomdp.fsc.Fsc | paynt.pomdp.fsc.FscFactored) -> paynt.colored_mdp.ColoredMdp:
        ...
        self.fsc_unfolder = payntbind.synthesis.FscUnfolder(self.underlying_mdp, self.state_to_observation, self.num_actions, self.choice_to_action)
        ...
        return paynt.colored_mdp.ColoredMdp(product, product_parameter_space, product_coloring, use_exact=self.use_exact)
