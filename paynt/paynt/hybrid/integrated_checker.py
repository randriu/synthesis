...
class IntegratedChecker(QuotientBasedFamilyChecker, CEGISChecker):
    ...
    def cegis_analyze_member_assigment(self, process_order_number, member_assignment, family, relevant_holes, counterexample_generator, return_process):
        ...
        Profiler.start("CEGIS worker - DTMC model checking")
        ...
        Profiler.start("CEGIS worker - prepare DTMC")
        counterexample_generator.prepare_dtmc(family.dtmc, family.dtmc_state_map)
        Profiler.stop()

        Profiler.start("CEGIS worker - find conflicts CE")
        conflicts = []
    ...
    def analyze_family_cegis(self, family):
        ...
        while not None in member_assignments:
            ...
            for i in range(Globals.CEGIS_CPU_COUNT):
                ...
                Profiler.start("CEGIS master - pick DTMC")
