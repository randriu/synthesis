...
from ..hybrid.helpers import safe_division
...
class Statistic:
    ...
    def __init__(self, sketch, method_name):
        ...
        self.formulae = sketch.formulae
        self.optimality_formula = sketch.optimality_formula
    ...
    def status(self):
        ...
        return f"> Processed {percentage_rejected}% members, elapsed / estimated time: {time_elapsed} / {time_estimate} s"
    ...
    def finished(self, assignment, optimal_value):
        self.timer.stop()
    ...
    def get_long_summary(self):
        formulae = "\n".join([f"formula {i + 1}: {str(f)}" for i,f in enumerate(self.formulae)]) + "\n"
        formulae += f"optimality formula: {str(self.optimality_formula)}\n" if self.optimality_formula is not None else ""
        ...
        mdp_stats = f"super MDP size: {self.super_mdp_size}, average MDP size: {round(self.avg_size_mdp)}, " \
                    f"MPD checks: {self.check_mdp}, iterations: {self.iterations_mdp}"
