    ...
    def verify_parameter_space(self, node):
        ...
        if node.analysis_result.improving_value is not None:
            if self.saynt_timer is not None:
                ...
                self.stat.new_fsc_found(node.analysis_result.improving_value, node.analysis_result.improving_assignment, self.colored_mdp.policy_size(node.analysis_result.improving_assignment))
