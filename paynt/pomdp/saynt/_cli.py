...
options = [
    click.option(
        "--storm-pomdp", is_flag=True, default=False, panel=_PANEL, help="enable running belief analysis in STorm to enhance FSC synthesis for POMDPs (AR only)"
    ),
    click.option(
        "--storm-options",
        default="cutoff",
    ...
    click.option(
        ...
        panel=_PANEL,
        help=(
            "runs PAYNT for given amount of seconds and returns Storm result using FSC at cutoff. If time is "
            "0 returns pure Storm result. Can only be used together with --storm-pomdp flag"
        ),
    ),
    click.option(
        "--prune-storm",
