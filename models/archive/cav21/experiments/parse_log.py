...
res = match(r"^.*?Sketch has (.*?) holes:.*?$", lines_start)
if res is not None:
    number_of_holes = res[0]

res = match(r"^.*?Design space.*?: (.*?)$", lines_start)
if res is not None:
    family_size = res[0]
...
res = match(r"^ce quality: maxsat: (.*?), trivial: (.*?), non-trivial: (.*?)$", lines_end)
if res is not None:
    ce_quality_maxsat = res[0]
    ce_quality_trivial = res[1]
    ce_quality_nontrivial = res[2]

res = match(r"^ce time: maxsat: (.*?), trivial: (.*?), non-trivial: (.*?)$", lines_end)
if res is not None:
